...
public class DigitalWheelView extends FrameLayout {
    ...
    public class DigitalGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            viewPager2.beginFakeDrag();
            viewPager2.fakeDragBy(-distanceY);
            return true;
        }
    }
    ...
    public DigitalWheelView(@NonNull Context context, @Nullable AttributeSet attributeSet, int defStyleAttr) {
        super(context, attributeSet, defStyleAttr);
        init(context);
    }

    private void init(@NonNull Context context) {
        LayoutInflater.from(context).inflate(R.layout.view_digital_wheel, this, true);
        ...
        this.viewPager2.setPageTransformer((page, position) -> {
            page.setTranslationY((-(((float) viewPager2.getHeight()) * 0.25f)) * position);
    ...
    public String getSelectedData() {
        ...
        if (list.size() != 0 && (currentItem = this.viewPager2.getCurrentItem()) < list.size()) {
